...
8. `exp_0139`/`exp_0140` leave V1 20d vol targeting as the only current position-stability scheme with live opt-in support. Its rolling12 improvement is real under a clean train-only vol_ref, but the residual max DD is low-volatility and high-size, so do not tune clip/window from this audit. V2 ATR risk parity should stay skipped unless Stage 0 evidence changes.
