#!/usr/bin/env python3
"""Diagnostic-only scan for BEAR -> NEUTRAL local transition degradation."""
...
OUTPUT_DIR = PROJECT_DIR / "research_workspace" / "b2n_transition_scan"
NOTES_MD = OUTPUT_DIR / "b2n_transition_local_degradation_scan_v0.md"
EVENTS_TSV = OUTPUT_DIR / "b2n_transition_events.tsv"
...
ALLOWED_CONCLUSIONS = (
    "localized_transition_pathology_found",
    "diffuse_or_noisy",
...
def _b2n_steps(regimes: np.ndarray) -> List[int]:
    labels = np.asarray(regimes, dtype=object)
    return [
        i for i in range(1, len(labels))
        if str(labels[i - 1]) == "BEAR" and str(labels[i]) == "NEUTRAL"
    ]
...
def _classify_transition(pre_position: int, post_position: int) -> str:
    if pre_position == 0 and post_position != 0:
        return "flat_entry"
    if pre_position != 0 and post_position == 0:
        return "inherited_exit"
    if pre_position != 0 and post_position == -pre_position:
...
def _event_rows(
    ...
    for idx, step in enumerate(_b2n_steps(regimes), start=1):
...
def _make_conclusion(summary: Dict[str, Any], events: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    concentrated_type = dominant_count <= 3 and dominant_count >= 2
    local_bad = (
    ...
    longer_recovery = (
        h7["mean_return"] is not None
        and h14["mean_return"] is not None
        and h7["mean_return"] > 0
        and h14["mean_return"] > 0
    )
    if local_bad and concentrated_type:
        return {
            "label": "localized_transition_pathology_found",
            "reason": (
                "B2N has negative 1d/3d local means with a concentrated event type. "
                "This is diagnosis only; no helper or contract is implied."
            ),
        }
    if h1["mean_return"] is not None and h1["mean_return"] < 0 and longer_recovery:
        return {
            "label": "no_actionable_signal",
            "reason": (
                "B2N has only a weak 1d loss pocket, 3d/7d/14d recover, and there is "
                "no concentrated event type to cut."
            ),
        }
    return {
        "label": "no_actionable_signal",
        "reason": "B2N does not show a repeated local degradation pattern worth opening.",
    }
    ...
    for days in HORIZON_DAYS:
        fields.extend(
            [
                f"h{days}_return",
                f"h{days}_dd",
                f"h{days}_equity_delta",
                f"h{days}_price_return",
            ]
        )
...
def _write_notes(path: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# B2N Transition Local Degradation Scan v0",
        "",
        f"Generated: {report['generated_at']}",
        "",
        "## Scope",
        "",
        "- Diagnostic-only scan for BEAR -> NEUTRAL transitions.",
    ...
    lines.extend(
        [
            ...
            "",
            f"- B2N transition count: {summary['transition_count']}",
            ...
            "## Guardrail",
            "",
            "- This scan only answers whether B2N has a local pathology.",
...
def _write_no_touch(path: Path, report: Dict[str, Any]) -> None:
    lines = [
        "# No-Touch Audit: B2N Transition Local Degradation Scan",
        "",
        ...
        "",
        "- `scripts/b2n_transition_local_degradation_scan.py`",
        "- `research_workspace/b2n_transition_scan/b2n_transition_local_degradation_scan_v0.md`",
        "- `research_workspace/b2n_transition_scan/b2n_transition_events.tsv`",
        "- `research_workspace/b2n_transition_scan/no_touch_audit.md`",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_scan(
    *,
    ...
    conclusion = _make_conclusion(summary, events)
    return {
        "generated_at": _now_iso(),
        "scan_id": "B2N_transition_local_degradation_scan_v0",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diagnostic-only B2N transition scan.")
    ...
    print("B2N transition local degradation scan complete.")
